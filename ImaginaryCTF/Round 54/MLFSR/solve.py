...
def estimate_ortho_size(n, t, m):
    ...
    rkb = reduce_mod_p(mat.right_kernel_matrix(), p)
...
cvps, bs = solve_inequality_ex(L, lb, ub)
assert (
    cvps == 0
)  # because the average of lb and ub are zero, so its cvp are zero vector, which is useless
assert (
    bs[: t + 5, MM.nrows() :].change_ring(GF(p)).rank() == t
), "recover failed"  # if the recovered short vectors are correct, it should have exactly rank t
rec = (
